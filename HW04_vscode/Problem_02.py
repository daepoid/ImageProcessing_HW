...
def read_raw_img(filepath):
    ...
    with open(filepath, 'rb') as bmp:
        raw_image = struct.unpack('B' * 512 * 512, bmp.read(512 * 512))
    return list(raw_image)


def reverse_img(raw_image):
    for i in range(MATRIX_MAX):
        for j in range(MATRIX_MAX):
            if i < (MATRIX_MAX / 2):
                raw_image[i * MATRIX_MAX + j], raw_image[(MATRIX_MAX - i - 1) * MATRIX_MAX + j] = raw_image[(
                    MATRIX_MAX - i - 1) * MATRIX_MAX + j], raw_image[i * MATRIX_MAX + j]
...
def main():
    ...
    new_img_path = 'outputs/Restored_BOAT512_py.bmp'
    BMPHEADERS, _ = read_bmp_img(input_file)

    input_file = 'BOAT512.raw'
    raw_image = read_raw_img(input_file)
