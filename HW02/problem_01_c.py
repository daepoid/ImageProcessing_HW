...
def basic_contrast_stretching(raw_data):
    hist_data = [0 for i in range(len(raw_data))]
    min_val = 256
    max_val = -1

    for i in range(len(raw_data)):
        if min_val > raw_data[i]:
            min_val = raw_data[i]
        if max_val < raw_data[i]:
            max_val = raw_data[i]
...
def main():
    ...
    output_graph_path = 'outputs/problem_01_c'
    new_img_path = 'outputs/c_basic_contrast_stretching_lena.bmp'

    BMPHEADERS, raw_data = get_bmp_data(input_file)
    hist_data = basic_contrast_stretching(raw_data)
